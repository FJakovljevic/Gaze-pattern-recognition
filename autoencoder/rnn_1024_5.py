...
# define model
model = Sequential()
model.add(LSTM(1024, activation='tanh', input_shape=(reg_num,one_hot_len), return_sequences=True))
...
checkpoint = ModelCheckpoint('rnn_1024_5-{epoch:03d}-{acc:03f}-{val_acc:03f}.h5', verbose=0, monitor='val_loss',save_best_only=True, mode='auto') 
model.compile(optimizer='adam', loss='mse', metrics=['accuracy'])
print(model.summary())

# fit model
# model.fit(sequence, sequence, epochs=1000, verbose=1, validation_split=0.3, shuffle=True, batch_size=512)
history  = model.fit(sequence, sequence, epochs=500, verbose=1, batch_size=128, validation_split=0.1, shuffle=True, callbacks=[checkpoint])
